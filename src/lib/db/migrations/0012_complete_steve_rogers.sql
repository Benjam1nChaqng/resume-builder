CREATE TABLE "job_pipeline_event" (
...
ALTER TABLE "application" ADD COLUMN "notes" text;--> statement-breakpoint
...
CREATE INDEX "job_pipeline_event_user_job_idx" ON "job_pipeline_event" USING btree ("user_id","job_id","occurred_at");--> statement-breakpoint
CREATE INDEX "job_pipeline_event_listing_idx" ON "job_pipeline_event" USING btree ("listing_id","occurred_at");--> statement-breakpoint
CREATE FUNCTION "record_job_listing_pipeline_event"() RETURNS trigger AS $$
DECLARE
	"owner_user_id" text;
BEGIN
...
CREATE FUNCTION "record_direct_application_pipeline_event"() RETURNS trigger AS $$
BEGIN
