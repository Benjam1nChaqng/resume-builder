...
export async function fetchJobFeed(
  ...
  // Remotive is remote/professional only — skip it for purely local searches.
  ...
  try {
    const { ok, status, html } = await fetchExternalHtml(
      `${REMOTIVE_ENDPOINT}?${params.toString()}`,
      { headers: { Accept: "application/json" }, timeoutMs: 12_000 },
    );
    if (!ok) {
      return { listings: [], error: `Job feed returned HTTP ${status}` };
    }
    const payload = JSON.parse(html);
    return { listings: normalizeRemotiveJobs(payload, criteria), error: null };
  } catch (err) {
    return {
      listings: [],
